...
const matches = [];
for ( let item of arr) {
    item.manufacturer === mfrStr ? matches.push(`${item.title} was made by ${mfrStr} in ${item.year}`) : null;
}
return matches.length === 0 ? `We don't have anything by ${mfrStr} in our inventory` : matches;
};
